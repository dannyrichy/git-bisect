...
WEIGHT_PERM = VGG_PERM_PATH.joinpath("weight_perm_mnist.pkl")
ACT_PERM = VGG_PERM_PATH.joinpath("act_perm_mnist.pkl")
STE_PERM = VGG_PERM_PATH.joinpath("ste_perm_mnist.pkl")
VGG_MODEL1_PATH  = _STASH_PATH.joinpath("mnist_vgg16_bn1_100.pth")
VGG_MODEL2_PATH  = _STASH_PATH.joinpath("mnist_vgg16_bn2_100.pth")
...
def permute_model(
    ...
    permuted_model = vgg16_bn(num_classes=10, in_channels=1).to(DEVICE)
...
def activation_matching() -> dict[str, torch.Tensor]:
    ...
    train_loader, test_loader, _ = mnist_loader(batch_size=256)
    permuter = ActMatching(arch=LOOK_UP_LAYER)

    # Loading individually trained models
    vgg_model1, vgg_model2 = vgg16_bn(num_classes=10, in_channels=1), vgg16_bn(num_classes=10, in_channels=1)
...
def generate_plots(
    ...
    train_loader, test_loader, _ = mnist_loader(batch_size=128)
...
def run():
    ...
    plt_dict(results_dict, file_path=Path("vgg_mnist_results"))
