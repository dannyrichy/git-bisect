...
def mlp_width_ablation():
    ...
    for w in [64,128,256,512,1024]:
        ...
        plt.savefig(parent_folder.joinpath(f"mlp_width_{str(w)}"))
        plt.clf()
...
def mlp_time_ablation():
    ...
    for t in range(1,41):
        ...
        plt.savefig(parent_folder.joinpath(f"mlp_epoch_{str(t)}"))
        plt.clf()
...
def mlp_act_batch_ablation():
    ...
    for t in range(31):
        print(f"Running for epoch time {t}")
        mlp_model1, mlp_model2 = MLP(), MLP()
        mlp_model1.load_state_dict(torch.load(_STASH_PATH.joinpath(f"mlp_512_{t}_1_40.pth")))
        mlp_model1.to(DEVICE)
        mlp_model1.eval()

        mlp_model2.load_state_dict(torch.load(_STASH_PATH.joinpath(f"mlp1_512_{t}_1_40.pth")))
        mlp_model2.to(DEVICE)
        mlp_model2.eval()
        ...
        plt.savefig(parent_folder.joinpath(f"mlp_act_batch_{str(t)}"))
        plt.clf()
